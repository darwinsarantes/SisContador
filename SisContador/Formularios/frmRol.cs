...
namespace SisContador.Formularios
{
    public partial class frmRol : Form
    {
        ...
        private string NOMBRE_ENTIDAD_PRIVILEGIO = "Rol";
        private string NOMBRE_ENTIDAD = "Administrar los diferentes tipos de cuentas";
        private string NOMBRE_LLAVE_PRIMARIA = "idRol";
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oRol.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oRol);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idRol like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkDescripcion) == false && Controles.IsNullOEmptyElControl(txtDescripcion) == false)
            {
                Where += string.Format(" and Descripcion like '%{0}%' ", txtDescripcion.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkDescripcion) == false && Controles.IsNullOEmptyElControl(txtDescripcion) == false)
            {
                Titulo += string.Format(" Descripción: '{0}', ", txtDescripcion.Text.Trim());
            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmRolOperacion ofrmRolOperacion = new frmRolOperacion();
            ofrmRolOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmRolOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmRolOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmRolOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmRolOperacion.MdiParent = this.ParentForm;
            ofrmRolOperacion.Show();
        ...
        private void frmRol_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oRol, a);

                            oRol[a - 1] = new RolEN();
                            oRol[a - 1].idRol = Convert.ToInt32(Fila.Cells["idRol"].Value);
                            oRol[a - 1].Descripcion = Fila.Cells["Descripcion"].Value.ToString();
        ...
        private void frmRol_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDescripcion_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                chkDescripcion.CheckState = CheckState.Unchecked;
            }
            else { chkDescripcion.CheckState = CheckState.Checked; }

            if (chkDescripcion.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
