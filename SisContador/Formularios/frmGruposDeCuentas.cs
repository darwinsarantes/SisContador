...
{
    public partial class frmGruposDeCuentas : Form
    {
        public frmGruposDeCuentas()
        {
            InitializeComponent();
        }

        private string NOMBRE_ENTIDAD_PRIVILEGIO = "GrupoDecuentas";
        private string NOMBRE_ENTIDAD = "Administrar los grupos de cuentas";
        private string NOMBRE_LLAVE_PRIMARIA = "idGrupoDeCuentas";
        ...
        public GrupoDeCuentasEN[] oGrupoDeCuentas = new GrupoDeCuentasEN[0];
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oGrupoDeCuentas.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oGrupoDeCuentas);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idGrupoDeCuentas like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkDescGrupoDeCuentas) == false && Controles.IsNullOEmptyElControl(txtDescGrupoDeCuentas) == false)
            {
                Where += string.Format(" and DescGrupoDeCuentas like '%{0}%' ", txtDescGrupoDeCuentas.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkDebito) == false && Controles.IsNullOEmptyElControl(cmbDebito) == false)
            {
                Where += string.Format(" and trim(Debitos) = '{0}' ", cmbDebito.Text);
            }

            if (Controles.IsNullOEmptyElControl(chkCredito) == false && Controles.IsNullOEmptyElControl(cmbCredito) == false)
            {
                Where += string.Format(" and trim(Creditos) = '{0}' ", cmbCredito.Text);
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkDescGrupoDeCuentas) == false && Controles.IsNullOEmptyElControl(txtDescGrupoDeCuentas) == false)
            {
                Titulo += string.Format(" Descripción: '{0}', ", txtDescGrupoDeCuentas.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkDebito) == false && Controles.IsNullOEmptyElControl(cmbDebito) == false)
            {
                Titulo += string.Format(" Debitos: '{0}', ", cmbDebito.Text);
            }

            if (Controles.IsNullOEmptyElControl(chkCredito) == false && Controles.IsNullOEmptyElControl(cmbCredito) == false)
            {
                Titulo += string.Format(" Creditos: '{0}', ", cmbCredito.Text);
            }
        ...
        private void FormatearDGV()
        {
            try
            {
                ...
                string OcultarColumnas = "IdUsuarioDeCreacion, FechaDeCreacion, IdUsuarioDeModificacion, FechaDeModificacion";
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "GrupoDeCuentas");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmGruposDeCuentasOperacion ofrmGruposDeCuentasOperacion = new frmGruposDeCuentasOperacion();
            ofrmGruposDeCuentasOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmGruposDeCuentasOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmGruposDeCuentasOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmGruposDeCuentasOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmGruposDeCuentasOperacion.MdiParent = this.ParentForm;
            ofrmGruposDeCuentasOperacion.Show();
        ...
        private void frmGruposDeCuentas_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oGrupoDeCuentas, a);

                            oGrupoDeCuentas[a - 1] = new GrupoDeCuentasEN();
                            oGrupoDeCuentas[a - 1].idGrupoDeCuentas = Convert.ToInt32(Fila.Cells["idGrupoDeCuentas"].Value);
                            oGrupoDeCuentas[a - 1].DescGrupoDeCuentas = Fila.Cells["DescGrupoDeCuentas"].Value.ToString();
                            oGrupoDeCuentas[a - 1].Debitos = Fila.Cells["Debitos"].Value.ToString();
                            oGrupoDeCuentas[a - 1].Creditos = Fila.Cells["Creditos"].Value.ToString();
        ...
        private void frmGruposDeCuentas_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDescGrupoDeCuentas_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtDescGrupoDeCuentas))
            {
                chkDescGrupoDeCuentas.CheckState = CheckState.Unchecked;
            }
            else { chkDescGrupoDeCuentas.CheckState = CheckState.Checked; }

            if (chkDescGrupoDeCuentas.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
        ...
        private void cmbDebito_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            {
                chkDebito.CheckState = CheckState.Unchecked;
            }
            else { chkDebito.CheckState = CheckState.Checked; }

            if (chkDebito.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        ...
        private void cmbCredito_SelectionChangeCommitted(object sender, EventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(cmbCredito))
            {
                chkCredito.CheckState = CheckState.Unchecked;
            }
            else { chkCredito.CheckState = CheckState.Checked; }

            if (chkCredito.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
