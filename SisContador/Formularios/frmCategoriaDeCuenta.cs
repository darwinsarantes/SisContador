...
{
    public partial class frmCategoriaDeCuenta : Form
    {
        public frmCategoriaDeCuenta()
        {
            InitializeComponent();
        }

        private string NOMBRE_ENTIDAD_PRIVILEGIO = "CategoriaDeCuentas";
        private string NOMBRE_ENTIDAD = "Administrar las diferentes categorias de una cuenta";
        private string NOMBRE_LLAVE_PRIMARIA = "idCategoriaDeCuenta";
        ...
        public CategoriaDeCuentaEN[] oCategoriaDeCuenta = new CategoriaDeCuentaEN[0];
        ...
        private void AgregarRegistrosAlDTUsuario()
        {
            ...
            if (oCategoriaDeCuenta.Length > 0)
            {
        ...
        private DataTable ConvertirClassADT()
        {
            ...
            System.Xml.Serialization.XmlSerializer serializer = new System.Xml.Serialization.XmlSerializer(oCategoriaDeCuenta.GetType());
            System.IO.StringWriter sw = new System.IO.StringWriter();
            serializer.Serialize(sw, oCategoriaDeCuenta);
        ...
        private string WhereDinamico() {
            ...
            if (Controles.IsNullOEmptyElControl(chkIdentificador) == false && Controles.IsNullOEmptyElControl(txtIdentificador) == false) {
                Where += string.Format(" and idCategoriaDeCuenta like '%{0}%' ", txtIdentificador.Text.Trim());
            }

            if (Controles.IsNullOEmptyElControl(chkDescCategoriaDeCuenta) == false && Controles.IsNullOEmptyElControl(txtDescCategoriaDeCuenta) == false)
            {
                Where += string.Format(" and DescCategoriaDeCuenta like '%{0}%' ", txtDescCategoriaDeCuenta.Text.Trim());
            }
        ...
        private string TituloDinamico()
        {
            ...
            if (Controles.IsNullOEmptyElControl(chkDescCategoriaDeCuenta) == false && Controles.IsNullOEmptyElControl(txtDescCategoriaDeCuenta) == false)
            {
                Titulo += string.Format(" Descripción: '{0}', ", txtDescCategoriaDeCuenta.Text.Trim());
            }
        ...
        private void FormatearDGV()
        {
            try
            {
                ...
                string OcultarColumnas = "idGrupoDeCuentas,IdUsuarioDeCreacion, FechaDeCreacion, IdUsuarioDeModificacion, FechaDeModificacion";
                ...
                foreach (DataGridViewColumn c1 in dgvLista.Columns)
                {
                    if (c1.Visible == true)
                    {
                        if (c1.Name.Trim().ToUpper() != "Seleccionar".ToUpper())
                        {
                            ...
                            {
                                oFormato = new FormatoDGV(c1.Name.Trim(), "CategoriaDeCuenta");
                            }
        ...
        private void MostrarFormularioParaOperacion(string OperacionesARealizar)
        {
            ...
            frmCategoriaDeCuentaOperacion ofrmCategoriaDeCuentaOperacion = new frmCategoriaDeCuentaOperacion();
            ofrmCategoriaDeCuentaOperacion.OperacionARealizar = OperacionesARealizar;
            ofrmCategoriaDeCuentaOperacion.NOMBRE_ENTIDAD_PRIVILEGIO = NOMBRE_ENTIDAD_PRIVILEGIO;
            ofrmCategoriaDeCuentaOperacion.NombreEntidad = NOMBRE_ENTIDAD;
            ofrmCategoriaDeCuentaOperacion.ValorLlavePrimariaEntidad = this.ValorLlavePrimariaEntidad;
            ofrmCategoriaDeCuentaOperacion.MdiParent = this.ParentForm;
            ofrmCategoriaDeCuentaOperacion.Show();
        ...
        private void frmCategoriaDeCuenta_Shown(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dgvLista.Rows.Count > 0)
                {
                    foreach (DataGridViewRow Fila in dgvLista.Rows)
                    {
                        ...
                        {
                            a++;
                            Array.Resize(ref oCategoriaDeCuenta, a);

                            oCategoriaDeCuenta[a - 1] = new CategoriaDeCuentaEN();
                            oCategoriaDeCuenta[a - 1].idCategoriaDeCuenta = Convert.ToInt32(Fila.Cells["idCategoriaDeCuenta"].Value);
                            oCategoriaDeCuenta[a - 1].DescCategoriaDeCuenta = Fila.Cells["DescCategoriaDeCuenta"].Value.ToString();
                            oCategoriaDeCuenta[a - 1].oGrupoDeCuentasEN.idGrupoDeCuentas = Convert.ToInt32(Fila.Cells["idGrupoDeCuentas"].Value.ToString());
                            oCategoriaDeCuenta[a - 1].oGrupoDeCuentasEN.DescGrupoDeCuentas = Fila.Cells["DescGrupoDeCuentas"].Value.ToString();
        ...
        private void frmCategoriaDeCuenta_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void txtDescCategoriaDeCuenta_KeyUp(object sender, KeyEventArgs e)
        {
            if (Controles.IsNullOEmptyElControl(txtDescCategoriaDeCuenta))
            {
                chkDescCategoriaDeCuenta.CheckState = CheckState.Unchecked;
            }
            else { chkDescCategoriaDeCuenta.CheckState = CheckState.Checked; }

            if (chkDescCategoriaDeCuenta.CheckState == CheckState.Checked && tsbFiltroAutomatico.CheckState == CheckState.Checked)
            {
                LLenarListado();
            }
        }
        ...
        private void frmCategoriaDeCuenta_Enter(object sender, EventArgs e)
        {
            if (tsbFiltroAutomatico.CheckState == CheckState.Checked)
                LLenarListado();
        }
